...
class Calibration:
    ...
    def caliExcusion(self):
        ...
        while True:
            ...
            if not self.is_start:  ## Only play once Calibration
                calibration_thread = threading.Thread(target=self.start_cali, name='calibration_th')
                calibration_thread.daemon = True
                calibration_thread.start()

            if self.current_image is not None:
                cv2.imshow('vis', self.current_image)
